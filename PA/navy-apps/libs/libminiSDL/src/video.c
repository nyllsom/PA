...
static inline uint32_t color_translater(SDL_Color *c) {
...
//   for (int i = 0; i < h; i++) {
//     memcpy(
//       dst_pixels + (dst_y + i) * dst->w + dst_x,
//       src_pixels + (src_y + i) * src->w + src_x,
//       w * 4
//     );
//   }
// }
...
void SDL_UpdateRect(SDL_Surface *s, int x, int y, int w, int h) {
  ...
  if (s->format->BitsPerPixel == 32) {
    ...
    for (int i = 0; i < h; i++) {
      for (int j = 0; j < w; j++) {
        uint8_t idx = src[(y + i) * s->w + (x + j)];
        tmp[i * w + j] = color_translater(&pal[idx]);
      }
    }
  }
  else {
